	...
	for (pdi = min_pdi; pdi <= max_pdi; pdi++)
	{
		...
		if (pde & 0x1)
		{
			if (pde & 0x80)  // If it is a 4 MiB page...
			{
				...
				final_virtual_addr = (initial_virtual_addr + 0x3FFFFF);
				
		//C. i have to put the offset to 1 all the components , i guess i can make this with a mask

				// Initial physical address of the page
				initial_physical_addr = (pde & 0xFFC0000);

				// Final physical address of the page
				final_physical_addr = (initial_physical_addr + 0x3FFFFF);
			...
			{
				for (pti = 0x0; pti < 0x3FF; pti++) // For each PTI...
				{
					// Calculate the first virtual address with PDI = pdi and PTI = pti
					initial_virtual_addr = (void *)((pdi << 22) | (pti << 12)); 									//C.not really sure 22 12
					...
					{
						// Final virtual address of the page
						final_virtual_addr = (initial_virtual_addr + 0xFFF);

						// Initial physical address of the page
						initial_physical_addr = (pte & 0xFFFFF000);

						// Final physical address of the page
						final_physical_addr = (initial_physical_addr + 0xFFF);
